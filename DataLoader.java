        ...
        while (sc.hasNextLine()) {
            ...
            try {
                ...
                if (type.equals("E")) {
                    ...
                    invoice.addItem(new Equipment(template.getUuid(), template.getName(), template.getBaseRate(), status, quantity));
                
                } else if (type.equals("S")) {
                    Person tech = persons.get(t[2]);
                    double hours = Double.parseDouble(t[3]);
                    invoice.addItem(new Service(template.getUuid(), template.getName(), template.getBaseRate(), tech, hours));
                
                } else if (type.equals("L")) {
                    LocalDate start = LocalDate.parse(t[2]);
                    LocalDate end = LocalDate.parse(t[3]);
                    invoice.addItem(new License(template.getUuid(), template.getName(), template.getExtraFee(), template.getBaseRate(), start, end));
                }
            } catch (Exception ex) {
                System.err.println("Error loading invoice item for " + invoiceUuid);
            }
        }
        sc.close();
    }
